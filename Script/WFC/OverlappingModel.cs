...
class OverlappingModel : Model
{
    ...
    public OverlappingModel(Texture2D textureEntre, int N, int width, int height, bool periodicInput, bool periodic, int symmetry, bool ground, Heuristic heuristic)
        : base(width, height, N, periodic, heuristic)
    {
        //var (bitmap, SX, SY) = BitmapHelper.LoadBitmap($"samples/{name}.png");        
        bitmap = textureEntre.GetPixels();
        SX = textureEntre.height;
        SY = textureEntre.width;
        byte[] sample = new byte[bitmap.Length];
        colors = new List<int>();
        for (int i = 0; i < sample.Length; i++)
        {
            int color = int.Parse(ColorUtility.ToHtmlStringRGB(bitmap[i]));
            int k = 0;
    ...
    public void Save(Texture2D textureSortie)
    {
        ...
        textureSortie = new Texture2D(SY, SX, TextureFormat.RGBA32, false);
        byte[] arrayByte = new byte[bitmap.Length];
        for (int i = 0; i < bitmap.Length; i++)
        {
            arrayByte[i] = (byte)bitmap[i];
        }
        textureSortie.LoadRawTextureData(arrayByte);
    }
}